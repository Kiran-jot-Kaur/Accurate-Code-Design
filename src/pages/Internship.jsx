import PageWrapper from "../components/PageWrapper";

const Internship = () => {
  return (
    <PageWrapper>
      <div className="mx-auto max-w-7xl px-6 py-20">
        <div className="mb-16 text-center">
          <h1 className="text-4xl font-bold text-white">Internship Program</h1>
          <p className="mt-4 text-gray-400">
            Our internship program is designed to give students real product
            experience in a company-like environment.
          </p>
        </div>

        <div className="mb-20">
          <h2 className="mb-8 text-2xl font-semibold text-white">
            How Our Internship Works
          </h2>

          <div className="grid grid-cols-1 gap-8 md:grid-cols-4">
            <div className="rounded-lg border border-white p-6">
              <span className="text-sm font-semibold text-gray-400">
                Step 01
              </span>
              <h3 className="mt-3 mb-2 text-lg font-semibold text-white">
                Skill Readiness
              </h3>
              <p className="text-gray-400">
                Students first complete required training modules to ensure they
                are ready for real project work.
              </p>
            </div>

            <div className="rounded-lg border border-white p-6">
              <span className="text-sm font-semibold text-gray-400">
                Step 02
              </span>
              <h3 className="mt-3 mb-2 text-lg font-semibold text-white">
                Product Assignment
              </h3>
              <p className="text-gray-400">
                Interns are assigned to internal company products based on their
                skill level and interests.
              </p>
            </div>

            <div className="rounded-lg border border-white p-6">
              <span className="text-sm font-semibold text-gray-400">
                Step 03
              </span>
              <h3 className="mt-3 mb-2 text-lg font-semibold text-white">
                Team Collaboration
              </h3>
              <p className="text-gray-400">
                Interns work in small teams, use Git, receive reviews, and
                follow real development workflows.
              </p>
            </div>

            <div className="rounded-lg border border-white p-6">
              <span className="text-sm font-semibold text-gray-400">
                Step 04
              </span>
              <h3 className="mt-3 mb-2 text-lg font-semibold text-white">
                Internship Experience
              </h3>
              <p className="text-gray-400">
                Interns gain hands-on experience, project ownership, and
                exposure to real-world development practices.
              </p>
            </div>
          </div>
        </div>

        <div className="mb-20">
          <h2 className="mb-8 text-2xl font-semibold text-white">
            What Interns Will Work On
          </h2>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 border border-white rounded-lg p-6">
            <ul className="list-disc pl-6 text-gray-400">
              <li>Frontend UI development using React</li>
              <li>Building responsive and accessible interfaces</li>
              <li>Working with APIs and backend data</li>
              <li>Using AI tools for coding and debugging</li>
              <li>Understanding product requirements</li>
            </ul>

            <ul className="list-disc pl-6 text-gray-400">
              <li>Fixing bugs and improving UI performance</li>
              <li>Collaborating using Git and version control</li>
              <li>Participating in code reviews</li>
              <li>Learning deployment basics</li>
              <li>Documenting project work</li>
            </ul>
          </div>
        </div>

        <div className="mb-20 rounded-lg bg-gray-500 p-8">
          <h2 className="mb-4 text-2xl font-semibold text-white">
            Eligibility & Selection
          </h2>
          <p className="text-gray-900">
            Internship opportunities are offered to students who demonstrate
            commitment, learning progress, and discipline during training.
          </p>
          <p className="text-gray-900">
            Selection is based on performance, not on certificates or fees.
          </p>
        </div>

        <div className="text-center">
          <a
            href="/contact"
            className="rounded-md bg-gray-500 px-2 py-3 md:px-4 text-sm font-medium text-white hover:bg-gray-800 transition-colors duration-200 inline-block"
          >
            Apply for Internship Program
          </a>
        </div>
      </div>
    </PageWrapper>
  );
};

export default Internship;
